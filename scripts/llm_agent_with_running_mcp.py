#!/usr/bin/env python3
"""LLM-backed agent that connects to an already-running MCP server endpoint."""
...
def parse_args() -> argparse.Namespace:
    """Parse command-line arguments for the LLM-backed agent."""
    parser = argparse.ArgumentParser(
        description="Use a local Ollama model to choose and execute a tool on a running MCP server."
    )
    parser.add_argument("prompt", help="Natural-language user request.")
    parser.add_argument(
        "--mcp-url",
        default=os.environ.get("PETSTORE_MCP_URL", "http://127.0.0.1:8765/mcp"),
        help="URL for running streamable-http MCP server (default: http://127.0.0.1:8765/mcp).",
    )
    parser.add_argument(
        "--ollama-url",
    ...
    parser.add_argument(
        ...
        type=float,
        default=30.0,
        help="Timeout in seconds for the MCP streamable-http client.",
    )
...
async def run_agent(args: argparse.Namespace) -> int:
    """Route prompt via LLM and execute tool on a running MCP server."""
    ...
    async with streamablehttp_client(args.mcp_url, timeout=args.mcp_timeout) as (
        read_stream,
        write_stream,
        _get_session_id,
...
def main() -> None:
    """CLI entrypoint for the LLM-backed MCP agent (external server mode)."""
